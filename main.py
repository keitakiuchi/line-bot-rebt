...
7. 出力の前に、出力しないメモとして、現在のフローステップ番号と名前を<flow>タブに、対話履歴における同じフロー・ステップの直近の継続回数<number>タブに、ユーザへのあなたの返答を<response>タブにいったん保持する。
...
回答の出力：
<response>タブ内に書かれたユーザへの返答のみ出力する。タブ自体や、他のタブの内容は出力しない。

対話を通して応答作成の手順を順守し、1つのフローステップを3回以上続けてください。

ユーザ入力: {{input}}
Response:[<response>タブの中身のみ]
"""
